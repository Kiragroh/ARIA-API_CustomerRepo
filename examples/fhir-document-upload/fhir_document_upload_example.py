...
def organization_reference(args: argparse.Namespace, session: requests.Session) -> str:
    names = [name.strip() for name in args.organization_name if name.strip()]
    if not names:
        raise SystemExit(
            "Mindestens ein --organization-name ist erforderlich. "
            "Provider-Organisationen werden nicht per generischem Fallback "
            "ausgewaehlt."
        )
    errors: list[str] = []
    for name in names:
